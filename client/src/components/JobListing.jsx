...
import { AppContext } from "../context/AppContext"; // Make sure the import path is correct
...
const JobListing = () => {
  const {
    ...
    searchFilter,
    setSearchFilter,
  } = useContext(AppContext); // Ensure you're accessing the correct context
  ...
  const filterJobs = () => {
    ...
    // Matches location filter
    const matchesLocation = (job) =>
      selectedLocations.length === 0 ||
      selectedLocations.includes(job.location);

    // Matches title filter
    const matchesTitle = (job) =>
      !searchFilter.title ||
      job.title.toLowerCase().includes(searchFilter.title.toLowerCase());

    // Matches location in the search filter
    const matchesSearchLocation = (job) =>
      !searchFilter.location ||
      job.location.toLowerCase().includes(searchFilter.location.toLowerCase());

    // Apply all filters, ensuring that if no category is selected, we still show jobs based on title and location
  ...
  useEffect(() => {
    // If no categories are selected, reset filteredJobs to the original list
    const newFilteredJobs =
      selectedCategories.length === 0 ? jobsData : filterJobs(); // Make sure to reference the original list (e.g., allJobs)
